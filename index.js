...
function init() {
  ...
  document.getElementById("num-contributions").innerHTML = GAMES_JSON.reduce(
    (sum, index) => {
  ...
  const displayStr = `A total of $${sum.toLocaleString()} has been raised for ${
    GAMES_JSON.length
  } games. Currently, ${filteredList.length} ${
    filteredList.length == 1 ? "game remains" : "games remain"
  } unfunded. We need your help to fund these amazing games!`;
  document.getElementById("display").innerHTML = displayStr;

  var sorted = GAMES_JSON.sort((a, b) => {
    return b.pledged < a.pledged ? -1 : b.pledged > a.pledged ? 1 : 0;
  });
